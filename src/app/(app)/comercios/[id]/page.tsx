...
export default function StoreDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <section className="store-detail-live">
      ...
      <article className="panel store-detail-limits"><div><small>Limite utilizado</small><strong>{formatCurrency(store.used)}</strong></div><div><small>Limite total</small><strong>{formatCurrency(store.limit)}</strong></div><ProgressBar value={percent} color={percent > 80 ? "#f59e0b" : "#16a085"} /><p><span>{Math.round(percent)}% utilizado</span><span>{purchases.length} compras cadastradas</span></p></article>
    </section>
